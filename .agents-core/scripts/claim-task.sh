...
if [ -n "$PROJECT" ]; then
    ...
    # Search all projects (directories in state/)
    PROJECTS=$(ls -1 state/ 2>/dev/null | grep -v '\.toon$')
fi

# Find highest priority ready task
for proj in $PROJECTS; do
    for file in state/$proj/tasks/TASK-*.toon; do
        if [ -f "$file" ]; then
...
mkdir -p "state/$TASK_PROJECT/claimed"

# Move file to claimed/
mv "$TASK_FILE" "state/$TASK_PROJECT/claimed/$TASK_ID.toon"

# Add claim metadata with heartbeat
CLAIM_TIME=$(date -u +%Y-%m-%dT%H:%M:%SZ)
cat >> "state/$TASK_PROJECT/claimed/$TASK_ID.toon" <<EOF
...
# Commit changes
git add "state/$TASK_PROJECT/claimed/$TASK_ID.toon" "state/$TASK_PROJECT/tasks/" 2>/dev/null || true
